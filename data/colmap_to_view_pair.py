...
def _find_number(str):
    return re.findall('[0-9]+', str)


def _calc_score(pair, images, points3d, extrinsic, image_ext, theta0, sigma1, sigma2):
    i, j = pair
    ...
    image_name_i = int(_find_number(images[i+1].name)[0])
    image_name_j = int(_find_number(images[j+1].name)[0])
