...
class Reg(nn.Module):
    ...
    def __init__(self, reg_name):
        ...
        self.unet = UNet(8, 1, 0, 4, [], [8, 16], [], reg_name, dim=3)
        self.final_conv = nn.Conv3d(8, 1, 3, 1, 1, bias=False)

    def forward(self, x):
        init = self.init_conv(x)
        interm = self.unet(init)
        out = self.final_conv(interm)
        return interm, out
...
class GNRefine(nn.Module):
    def __init__(self):
        ...
        self.sobel_conv = nn.Conv2d(
            32, 64, 3, padding=1, groups=32, bias=False)
    ...
    def forward(self, init_d, ref_feat, ref_cam, srcs_feat, srcs_cam, s_scale):
        ...
        for src_feat, src_cam_scaled in zip(srcs_feat, srcs_cam_scaled):
            ...
            with torch.no_grad():
                pixel_grids = get_pixel_grids(
                    *src_feat.size()[-2:]).unsqueeze(0)  # 1hw31
                warped_homo_coord = (H @ pixel_grids).squeeze(-1)  # nhw3
                warped_scale = warped_homo_coord[..., 2:3] + 1e-9  # nhw2
                warped_coord = warped_homo_coord[..., :2] / warped_scale
            warped = interpolate(src_feat, warped_coord)
            residual = (warped - ref_feat).permute(0,
                                                   2, 3, 1).unsqueeze(-1)  # nhwc1
            ...
            d3to2_1 = torch.eye(2, dtype=torch.float32).cuda().view(
                1, 1, 1, 2, 2) / (warped_homo_coord[..., -1].view(n, h, w, 1, 1)+1e-9)  # nhw22
...
class SingleStage(nn.Module):
    ...
    def __init__(self):
        ...
        self.reg_pair = Reg('reg_pair')
        self.reg_fuse = Reg('reg_fuse')
    ...
    def build_cost_volume(self, ref, ref_cam, src, src_cam, depth_num, depth_start, depth_interval, s_scale, d_scale):
        ref_cam_scaled, src_cam_scaled = [scale_camera(
            cam, 1 / s_scale) for cam in [ref_cam, src_cam]]
        Hs = get_homographies(ref_cam_scaled, src_cam_scaled,
                              depth_num//d_scale, depth_start, depth_interval*d_scale)
        # ndhw33
        src_nd_c_h_w = src.unsqueeze(1).repeat(
            1, depth_num//d_scale, 1, 1, 1).view(-1, *src.size()[1:])  # n*d chw
        warped_src_nd_c_h_w = homography_warping(
            src_nd_c_h_w, Hs.view(-1, *Hs.size()[2:]))  # n*d chw
        warped_src = warped_src_nd_c_h_w.view(-1, depth_num//d_scale, *src.size()[
                                              1:]).transpose(1, 2)  # ncdhw
        return warped_src
    ...
    def forward(self, sample, depth_start, depth_num, depth_interval, upsample=False, mem=False, mode='soft', s_scale=1):
        ...
        ref_ncdhw = ref_feat.unsqueeze(2).repeat(
            1, 1, depth_num//d_scale, 1, 1)
        pair_results = []  # MVS

        if mode == 'soft':
            weight_sum = torch.zeros((ref_ncdhw.size()[0], 1, 1, ref_ncdhw.size()[
                                     3]//interm_scale, ref_ncdhw.size()[4]//interm_scale)).to(ref_ncdhw.dtype).cuda()
        if mode == 'hard':
            weight_sum = torch.zeros((ref_ncdhw.size()[0], 1, 1, ref_ncdhw.size()[
                                     3]//interm_scale, ref_ncdhw.size()[4]//interm_scale)).to(ref_ncdhw.dtype).cuda()
        if mode == 'average':
        ...
        for src_feat, src_cam in zip(srcs_feat, srcs_cam):
            warped_src = self.build_cost_volume(
                ref_feat, ref_cam, src_feat, src_cam, depth_num, depth_start, depth_interval, s_scale, d_scale)
            cost_volume = groupwise_correlation(ref_ncdhw, warped_src, 8, 1)
            interm, score_volume = self.reg_pair(cost_volume)
        ...
        interm, score_volume = self.reg_fuse(fused_interm)
        if d_scale != 1:
            score_volume = F.interpolate(score_volume, scale_factor=(
                d_scale, 1, 1), mode='trilinear', align_corners=False)
        score_volume = score_volume.squeeze(1)  # ndhw
        if upsample:
            score_volume = F.interpolate(
                score_volume, scale_factor=upsample_scale, mode='bilinear', align_corners=False)
...
class Model(nn.Module):
    ...
    def forward(self, sample, depth_nums, interval_scales, upsample=False, mem=False, mode='soft'):
        ...
        return pair_results, refined_depth, [prob_map_1_up, prob_map_2_up, prob_map_3]
...
class Loss(nn.Module):
    ...
    def forward(self, outputs, gt, masks, ref_cam, occ_guide=False, mode='soft'):  # MVS
        ...
        depth_end = depth_start + (depth_number - 2) * depth_interval
        ...
        for est_depth, pair_results in outputs:
            ...
            in_range = torch.min(
                (gt_downsized >= depth_start),
                (gt_downsized <= depth_end)
            )
            masks_valid = [
                torch.min(
                    (mask > 50), in_range
                ) for mask in masks_downsized
            ]  # mask and in_range
            masks_overlap = [
                torch.min(
                    (mask > 200), in_range
            ...
            union_overlap = bin_op_reduce(
                masks_overlap, torch.max)  # A(B+C)=AB+AC
            valid = union_overlap if occ_guide else in_range

            same_height = est_depth.size()[2] == pair_results[0][0].size()[2]
            same_width = est_depth.size()[3] == pair_results[0][0].size()[3]
            same_size = same_height and same_width
            ...
            in_range_interm = torch.min(
                ...
                (gt_interm <= depth_end)
            ) if not same_size else in_range
            masks_valid_interm = [
            ...
            union_overlap_interm = bin_op_reduce(
                masks_overlap_interm, torch.max
            ) if not same_size else union_overlap  # A(B+C)=AB+AC
            valid_interm = (
                union_overlap_interm if occ_guide else in_range_interm
            ) if not same_size else valid

            abs_err = (est_depth - gt_downsized).abs()
            abs_err_scaled = abs_err / depth_interval
            pair_abs_err = [(est - gt_interm).abs()
            ...
            if mode in ['soft', 'hard', 'uwta']:
                uncert_loss_zip = zip(
                    pair_abs_err_scaled, pair_results, masks_valid_interm, masks_overlap_interm)

                def uncert_loss(err, uncert, idx):
                    return (err[idx] * (-uncert[idx]).exp() + uncert[idx]).mean()

                if occ_guide:
                    uncert_losses = [
                        uncert_loss(err, uncert, mask_valid)
                        for err, (est, (uncert, occ)), mask_valid, mask_overlap in uncert_loss_zip
                    ]
                else:
                    uncert_losses = [
                        uncert_loss(err, uncert, in_range_interm)
            ...
            if occ_guide and mode in ['soft', 'hard', 'uwta']:
                logistic_loss_zip = zip(
                    pair_results, masks_valid_interm, masks_overlap_interm)

                def soft_margin_loss(occ, mask_overlap, mask_valid):
                    return nn.SoftMarginLoss()(occ[mask_valid], -mask_overlap[mask_valid].to(gt.dtype) * 2 + 1)

                logistic_losses = [
                    soft_margin_loss(occ, mask_overlap, mask_valid)
                    for (est, (uncert, occ)), mask_valid, mask_overlap in logistic_loss_zip
                ]
            ...
            if mode in ['soft', 'hard', 'uwta']:
                pair_loss = pair_loss + uncert_loss + \
                    (logistic_loss if occ_guide else 0.0)
            loss = l1 + pair_loss
